...
#define CLIENT_WIDTH		900		    // --- クライアント領域のデフォルトの幅, フルスクリーンの場合は水平方向解像度
...
//---- ゲーム処理				
int SetServerClient			( void );					// --- ゲーム処理			
int ChoseServerClient		( void );	
int Sound_Init				( void );					// --- 音楽
int Chara_player_init		( void );					// --- プレーヤーセット
int Chara_goblin_init		( void );					// --- 敵セット
int Chara_player_fall		( void );					// --- 地面チェック
int Chara_goblin_fall		( void );					// --- 地面チェック
int Chara_player_Anim		( void );					// --- アニメーション
int Chara_goblin_Anim		( void );					// --- アニメーション
...
// --- 通信関連データ
extern SERVER_SEND_DATA		server_send_work;								// --- サーバー送信データ
extern CLIENT_RECV_DATA		client_recv_work;								// --- クライアント受信データ
extern SERVER_RECV_DATA		server_recv_work;								// --- サーバー受信データ
extern CLIENT_SEND_DATA		client_send_work;								// --- クライアント送信データ
...
extern	int gbanim_nutral,gbanim_damage,gbanim_run ;

extern	int gamemode ;	// 全体的なゲームの管理
